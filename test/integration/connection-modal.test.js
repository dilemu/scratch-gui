...
// The tests below require DBit+ Link to be unavailable, so we can trigger
...
describe('Hardware extension connection modal', () => {
    ...
    test('Message saying DBit+ Link is unavailable (BLE)', async () => {
        await driver.quit();
        driver = getDriver();
        ...
        findByText('DBit+ Link'); // DBit+ Link is mentioned in the error modal

        const logs = await getLogs();
        await expect(logs).toEqual([]);
    });

    test('Message saying DBit+ Link is unavailable (BT)', async () => {
        await loadUri(uri);
